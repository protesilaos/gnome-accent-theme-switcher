...
```elisp
(use-package gnome-accent-color-switcher
  :init
  (unless (package-installed-p 'gnome-accent-color-switcher)
    (package-vc-install "https://github.com/protesilaos/gnome-accent-color-switcher.git"))
  :config
  (gnome-accent-color-switcher-mode 1))
```
